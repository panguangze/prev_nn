# scripts/06_rank_designs_FINAL.py
import os
import json
import glob
import numpy as np
...
# --- 2. 路径设置 ---
#base_dir = P["paths"]["predictions_dir"]
base_dir = os.path.join(P["paths"]["work_dir"], "af2_models", "predictions")
...
def calculate_interface_pae(pae_json_path, target_len, binder_len, design_name=""):
    """
    计算链间平均PAE。
    此函数经过优化，可以处理两种常见的PAE键名：'pae' 和 'predicted_aligned_error'。
    """
    ...
    # --- 核心修正：同时检查 'pae' 和 'predicted_aligned_error' ---
    pae_data = data.get('pae')
    if not pae_data: # 如果 'pae' 不存在或为空列表
        pae_data = data.get('predicted_aligned_error', []) # 则尝试获取 'predicted_aligned_error'

    pae_matrix = np.array(pae_data)

    if pae_matrix.size == 0:
        # 这种情况现在只会在两种键都找不到或都为空时发生
        print(f"  - 提示 ({design_name}): PAE JSON文件存在，但内部的 'pae' 或 'predicted_aligned_error' 列表为空。可能是不结合的预测。")
        return float('inf')
    ...
    block1 = pae_matrix[:target_len, target_len:]
    block2 = pae_matrix[target_len:, :target_len]

    if block1.size == 0 or block2.size == 0:
        print(f"  - 警告 ({design_name}): PAE矩阵切片后为空。")
        return float('inf')

    inter_pae_mean = (np.mean(block1) + np.mean(block2)) / 2
    return float(inter_pae_mean)
...
# 2. 自动扫描所有模型文件夹
print(f"\n正在扫描目录 '{base_dir}' 下的所有模型...")
model_dirs = [d for d in glob.glob(os.path.join(base_dir, '*')) if os.path.isdir(d)]
if not model_dirs:
    print(f"警告: 在 '{base_dir}' 中未找到任何模型子目录。请检查 `config/params.yaml` 中的 `predictions_dir` 路径。")
    exit()
print(f"找到了 {len(model_dirs)} 个模型目录，开始分析...")

all_results = []
for model_dir in tqdm(model_dirs, desc="分析所有模型中"):
    design_name = os.path.basename(model_dir)

    # 精确查找文件
    score_json = next(glob.iglob(os.path.join(model_dir, "*_scores_rank_001*.json")), None)
    pdb_file = next(glob.iglob(os.path.join(model_dir, "*_relaxed_rank_001*.pdb")), None)
    if not pdb_file:
        pdb_file = next(glob.iglob(os.path.join(model_dir, "*_unrelaxed_rank_001*.pdb")), None)
    pae_json = next(glob.iglob(os.path.join(model_dir, "*_predicted_aligned_error*.json")), None)
